...
class MIouMacro(IOUMacro):
    ...
    def value(self):
        ...
        for c in self.count:
            binary_iou_scores = np.append(
                binary_iou_scores,
                self.macro[c] / self.count[c] if self.count[c] > 0 else np.nan,
            )
        return np.nanmean(binary_iou_scores)
...
def display_diagnostics(learner, dls=None, return_vals=False):
    ...
    countdf = (
        ...
        .fillna(0)
        .astype(int)
        .rename(index=params.BDD_CLASSES)
    )
    ...
    disp.ax_.set_title(
        "Confusion Matrix (by Pixels)",
        fontdict={"fontsize": 32, "fontweight": "medium"},
    )
    fig.show()
